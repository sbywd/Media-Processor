...
struct ProcessingProperties {
    var resolutionEnabled: Bool, frameRateEnabled: Bool, videoBitrateEnabled: Bool, audioBitrateEnabled: Bool, containerEnabled: Bool, subtitleEnabled: Bool, exportAudioOnlyEnabled: Bool, exportVideoOnlyEnabled: Bool
...
class VideoItem: ObservableObject, Identifiable, Equatable {
    ...
    // --- 处理设置 ---
    @Published var resolutionEnabled: Bool = true
    @Published var frameRateEnabled: Bool = true
    @Published var videoBitrateEnabled: Bool = true
    @Published var audioBitrateEnabled: Bool = true
    @Published var containerEnabled: Bool = true
    ...
    var processingSummary: String {
        ...
        if resolutionEnabled { enabledItems.append("分辨率") }
        if frameRateEnabled { enabledItems.append("帧速率") }
        if videoBitrateEnabled { enabledItems.append("视频比特率") }
        if audioBitrateEnabled { enabledItems.append("音频比特率") }
        if containerEnabled { enabledItems.append("容器格式") }
        if subtitleEnabled { enabledItems.append("字幕") }
        ...
        default:
            ...
            return "\(firstTwo) 和另外 \(otherCount) 个处理项"
        }
    }
    
    var properties: ProcessingProperties {
        ProcessingProperties(resolutionEnabled: resolutionEnabled, frameRateEnabled: frameRateEnabled, videoBitrateEnabled: videoBitrateEnabled, audioBitrateEnabled: audioBitrateEnabled, containerEnabled: containerEnabled, subtitleEnabled: subtitleEnabled, exportAudioOnlyEnabled: exportAudioOnlyEnabled, exportVideoOnlyEnabled: exportVideoOnlyEnabled, resolutionWidth: resolutionWidth, resolutionHeight: resolutionHeight, keepAspectRatio: keepAspectRatio, frameRate: frameRate, videoBitrate: videoBitrate, videoEncoder: videoEncoder, audioBitrate: audioBitrate, audioEncoder: audioEncoder, containerFormat: containerFormat, subtitleFilePath: subtitleFilePath, subtitlePosition: subtitlePosition, subtitleColor: subtitleColor, subtitleFont: subtitleFont, subtitleFontSize: subtitleFontSize, subtitleOutlineColor: subtitleOutlineColor, subtitleOutlineWidth: subtitleOutlineWidth)
    }
    
    func apply(properties: ProcessingProperties) {
        resolutionEnabled = properties.resolutionEnabled; frameRateEnabled = properties.frameRateEnabled; videoBitrateEnabled = properties.videoBitrateEnabled; audioBitrateEnabled = properties.audioBitrateEnabled; containerEnabled = properties.containerEnabled; subtitleEnabled = properties.subtitleEnabled; exportAudioOnlyEnabled = properties.exportAudioOnlyEnabled; exportVideoOnlyEnabled = properties.exportVideoOnlyEnabled; resolutionWidth = properties.resolutionWidth; resolutionHeight = properties.resolutionHeight; keepAspectRatio = properties.keepAspectRatio; frameRate = properties.frameRate; videoBitrate = properties.videoBitrate; videoEncoder = properties.videoEncoder; audioBitrate = properties.audioBitrate; audioEncoder = properties.audioEncoder; containerFormat = properties.containerFormat; subtitleFilePath = properties.subtitleFilePath; subtitlePosition = properties.subtitlePosition; subtitleColor = properties.subtitleColor; subtitleFont = properties.subtitleFont; subtitleFontSize = properties.subtitleFontSize; subtitleOutlineColor = properties.subtitleOutlineColor; subtitleOutlineWidth = properties.subtitleOutlineWidth
    }
            ...
            DispatchQueue.main.async {
                ...
                case .success(let probeData):
                    ...
                    if let videoStream = probeData.streams.first(where: { $0.codec_type == "video" }) {
                        ...
                        self.videoInfo = "视频: \(width)x\(height) | \(codecNameForInfo) | \(bitrateKbps)Kbps | \(self.frameRate)FPS"
                    } else {
                        self.videoInfo = "视频: 未找到视频流"
                    }
                    ...
                    if let audioStream = probeData.streams.first(where: { $0.codec_type == "audio" }) {
                        ...
                        self.audioInfo = "音频: \(codecName) | \(bitrateKbps)Kbps"
                    } else {
                        self.audioInfo = "音频: 未找到音频流"
                    }

                case .failure(let error):
                    self.videoInfo = "错误: 无法读取元数据"
    ...
    func buildCommand(outputDirectory: URL) -> String {
        ...
        let outputExtension = containerEnabled ? containerFormat.lowercased() : url.pathExtension
        ...
        if exportAudioOnlyEnabled {
        ...
        } else if exportVideoOnlyEnabled {
            components.append("-an") // 无音频
            // --- 视频设置 (仅视频模式) ---
            let needsVideoReEncoding = resolutionEnabled || frameRateEnabled || videoBitrateEnabled || subtitleEnabled
            ...
            if audioBitrateEnabled {
                ...
                components.append("-c:a copy")
            }
        }
        
        if !videoFilters.isEmpty {
            components.append("-vf \"\(videoFilters.joined(separator: ","))\"")
        }
